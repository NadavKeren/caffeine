    ...
    public void clear() {
        for (int i = 0; i < maxLists; ++i) {
            ...
            while (sentinel.size > 0) {
                releaseToNodePool(requireNonNull(sentinel.next));
            }
        }
...
//        validateBlock();
    ...
    private static Node getNode(EntryData entry, Node inSentinel) {
        return (s_node_pool.size > 0 && s_node_pool.next != null)
    ...
    public void addEntry(AccessEvent event) {
    ...
    private Node getVictim() {
        ...
        for (int i : activeLists) {
            ...
            assert currVictim != null && currVictim.data != null;
    ...
    public void setSize(int size) {
        Assert.assertCondition(this.data.isEmpty() && size == 0,
    ...
    protected static final class Node {
        ...
        public Node setData(@Nullable EntryData data) {
